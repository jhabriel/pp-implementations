...
% Equation in a pseudo-one dimensional domain using Automatic Differentiation 
...
% faces by determing the upstream flux).
% 
...
%% Clearing and cleaning workspace
clear; clc();
...
% Newton loop
    ...
    eq = hEq(h_ad,h_n0,h_m0,dt);   % calling equation   
    eq.val
    qq =  divF(q(h_ad,h_m0))
    qq.val
    the_rest = C(h_m0-zetaCentr) .* (h_ad - h_m0)
    the_rest.val
